Scaling [elastalink-3f10] With Kubernetes
===========================
...
Based on your TML solution [elastalink-3f10] - if you want to scale your application with Kubernetes - you will need to apply the following YAML files.

.. list-table::

   * - **YML File**
     - **Description**
   * - :ref:`elastalink-3f10.yml`
   ...
   * - :ref:`qdrant.yml`
     - This is the Qdrant deployment YAML.
       ...
       applied if using Step 9 DAG.
   * - :ref:`nginx-ingress-elastalink-3f10.yml`
     - If you are scaling your TML solution you must

       apply the nginx-ingresselastalink-3f10.yml; this yaml is 
...
.. code-block:: YAML
   ...
   kubectl apply -f kafka.yml -f secrets.yml -f mysql-storage.yml -f mysql-db-deployment.yml -f elastalink-3f10.yml

elastalink-3f10.yml
------------------------

.. important::
   Copy and Paste this YAML file: elastalink-3f10.yml - and save it locally.
...
.. code-block:: YAML
     ...
     spec:
       ...
       template:
         ...
         spec:
           containers:
           - name: elastalink-3f10
             image: maadsdocker/elastalink-3f10-amd64:latest
             volumeMounts:
             - name: dockerpath
   ...
   ---
     ...
     kind: Service
     metadata:
       name: elastalink-3f10-visualization-service
       labels:
         app: elastalink-3f10-visualization-service
     spec:
       type: ClusterIP
       ports:
...
.. code-block::
   ...
   kubectl port-forward deployment/elastalink-3f10 6060:6060

After you forward the ports then copy/paste the viusalization URL below and run your dashboard.

.. code-block::

   http://localhost:6060/iot-failure-elastalink.html?topic=iot-preprocess&offset=-1&groupid=&rollbackoffset=400&topictype=prediction&append=0&secure=1
...
.. important::
   ...
   **STEP 4:  Apply nginx-ingress-elastalink-3f10.yml to your kubernetes cluster.  First you need to save it locally then apply it:**

nginx-ingress-elastalink-3f10.yml
-------------
   ...
   .. code-block::
      ...
      kubectl apply -f nginx-ingress-elastalink-3f10.yml
...
.. code-block::
   ...
   http://tml.tss/viz/iot-failure-elastalink.html?topic=iot-preprocess&offset=-1&groupid=&rollbackoffset=400&topictype=prediction&append=0&secure=1
